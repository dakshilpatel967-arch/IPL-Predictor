...
const PredictScreen = ({ matchId, user, navigate, goBack }) => {
  ...
  return (
    <div className="px-4 pt-6 pb-40"><TopBar onBack={goBack} title="Make your picks"/><MatchHeader match={match}/>
      ...
      <Question title="Who will score the highest runs?" num={2} pts={BAT_PTS}>{batsmen.length===0?<EmptyCard text="Player options not yet added."/>:<div className="space-y-2">{batsmen.map(p=><OptionRow key={p.id} selected={bat===p.id} onClick={()=>setBat(p.id)} name={p.player_name}/>)}</div>}</Question>
